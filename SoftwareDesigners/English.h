...
DNS	（domain name System）：	域名系统DoS（denial of service）：	拒绝服务攻击
...
EJB（enterprise javabean）：		企 业 
Javabean ERD（entity-relationship  diagram）：	实体联系图
...
MAC   （media access control）：	介 质 访 问 控 制 
...
take advantage of this opportunity and extend the original sources with new and interesting 
features. In the mid-1980s, every computer manufacturer either provided or planned to provide 
a Unix-based operating system for its computers. Each company had chosen a particular version 
of Unix to start with, and then added various proprietary features. Although all of these 
...
a view of direct-attached, dedicated disks to each server while pulling out the reliability, 
backup, and disk management issues from single-server control. SAN is geared for raw performance 
and generally achieves better performance than direct- attached disks as a result of larger 
...
and guided by specific controllable processes. Privacy issues are also very different in an 
...
multiple continents working on the same project. It's not just financial pressures that come 
to bear and break developers into teams. The sheer size-raw lines of code-of many of today's 
projects means they're just too big for one group to handle on its own.And whether teams are 
...
preserve social equilibrium, not to "rock the boat." And,  for  them  the  greatest  
...
displaying various pros and cons, signed by the contributors. They are task-based rather 
than relationship-based in their underlying structure. It is easy to see that this kind 
...
version of the image itself.  A container can  also work  in  concert  with  a  terminal,  
...
stereoscopic hardware, subtle human body-language cues, such as eye gaze, are not obscured.
Viewers' ability to interact with one another while using the terminal is thus uncompromised.
However, easy interaction with the surface of the display itself is precluded by the size and
shape of this terminal, and the fact that viewers should stand several feet away from the 
display to get maximum immersion. To counteract these factors, we provide an auxiliary 15-inch
...
on the Net. They represent new platforms–always on, always with you–that are opening up 
...
than  waiting for it to be permanently written. This greatly reduces latency, which is often
the critical performance point in a system trying to improve fault tolerance.Implementing
immediate acknowledge caches requires carefully designed hardware, software, and protocols
for redundancy and error recovery in the various system components, such as multiple server
processors, multiple paths to the cache, and potentially multiple batteries.This is further
complicated if caching is performed at multiple layers in the storage system with each 
point immediately acknowledging the write.  It  quickly  becomes  difficult  to  prove  
strong statements about the overall reliability of the stored data.
